...
class TetrahedralConstitutionStatus(str, Enum):
    ...
    CONSTITUTIONALLY_ORIENTATION_DISTINCT = "constitutionally_orientation_distinct"
    ...
    *,
    ...
    references = tuple(
        [
            ...
            *(virtual_reference("LP", center) for _ in range(lone_pairs)),
        ]
    )
    if neighbor_orientation_constraints is not None and not isinstance(
        ...
        raise TypeError("Neighbor orientation constraints have an unsupported type.")
    ...
    if stabilizer is not None and not stabilizer.is_stereogenic:
        ...
        status = TetrahedralConstitutionStatus.CONSTITUTIONALLY_ORIENTATION_DISTINCT
...
def perceive_tetrahedral_stereo(
    ...
    *,
    ...
    if any(center < 0 or center >= working.GetNumAtoms() for center in excluded):
    ...
    if pending:
        ...
        if constraints.tetrahedral or constraints.planar or constraints.cumulene:
