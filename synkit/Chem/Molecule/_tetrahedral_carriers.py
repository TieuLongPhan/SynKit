...
def is_tetrahedral_carrier(atom: Chem.Atom) -> bool:
    ...
    if atom.GetIsAromatic():
        return False
    slots = atom.GetDegree() + hidden_hydrogen_count(atom) + lone_pair_count(atom)
    if slots != 4:
        return False
    if atom.GetAtomicNum() not in {7, 15, 16}:
        return atom.GetHybridization() == Chem.HybridizationType.SP3 and all(
            bond.GetBondType() == Chem.BondType.SINGLE for bond in atom.GetBonds()
        )
