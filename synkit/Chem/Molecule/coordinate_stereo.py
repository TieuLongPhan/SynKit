...
def cumulene_axis_stereo_from_geometry(
    ...
    *,
    ...
    if len(support.path) < 3 or (len(support.path) - 1) % 2:
        raise ValueError(
            "Axial cumulene geometry requires a nonzero even bond count."
        )
    if minimum_oriented_volume <= 0:
        raise ValueError("The minimum oriented volume must be positive.")
...
def tetrahedral_stereo_from_geometry(
    ...
    *,
    ...
    inferred = tuple(
        -sum(components) for components in zip(*material_vectors)
    )
    resolved = tuple(
        inferred if value is None else value for value in vectors
    )
    ...
    volume = (
        0.0
        if denominator == 0.0
        else _dot(_cross(first, second), third) / denominator
    )
    if abs(volume) < minimum_oriented_volume:
...
def extended_cis_trans_from_geometry(
    ...
    *,
    ...
    if min(required) < 0 or max(required) >= molecule.GetNumAtoms():
        raise ValueError(
            "Extended cis/trans geometry references an absent atom."
        )
    conformer = molecule.GetConformer(conformer_id)
    left, right = support.path[0], support.path[-1]
    axis = _vector(
    ...
    alignment = (
        0.0
        if denominator == 0.0
        else _dot(left_vector, right_vector) / denominator
    )
    if abs(alignment) < minimum_alignment:
        raise ValueError(
            "Extended cis/trans geometry is orientation-indeterminate."
        )
    oriented_right = (
        right_frame if alignment > 0 else tuple(reversed(right_frame))
    )
