...
from ._chirality_types import (
    ...
    connectivity_edge_match as _connectivity_edge_match,
    molecular_node_colours as _molecular_node_colours,
    molecular_node_match as _molecular_node_match,
)
...
def _parity_constraint_assignment(
    ...
    values = {center: 0 for center in descriptors if center not in variable_centers}
...
def _analyze_unresolved_tetrahedral_completions(
    ...
    *,
    ...
    if not unresolved or any(
        ...
        return None
    variable_centers = frozenset(int(info.centeredOn) + 1 for info in unresolved)
    ...
    if any(not isinstance(value, TetrahedralStereo) for value in registry.values()):
        return None
    descriptors = {value.center: value for value in registry.values()}
...
def assess_molecular_chirality(  # noqa: C901
    molecule: Chem.Mol,
    *,
    max_isomers: int = 256,
    stereo_complete: bool = True,
    ...
    if tetrahedral_analysis is not None:
        ...
        representatives[baseline_classification] = tetrahedral_analysis.baseline_smiles
        evaluated = 1
    ...
    analysis_is_decisive = tetrahedral_analysis is not None and (
        tetrahedral_analysis.achiral_witness is None or len(classifications) > 1
    )
    ...
    enumeration_exhausted = (
        ...
        and (theoretical <= max_isomers or evaluated < max_isomers)
    )
    if len(classifications) > 1:
