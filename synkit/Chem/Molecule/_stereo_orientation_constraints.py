...
def _tetrahedral_constraints(
    ...
    for atom in molecule.GetAtoms():
        ...
        references.extend(
            virtual_reference("H", center) for _ in range(_hidden_hydrogens(atom))
        )
...
def _planar_constraints(
    ...
    for bond in molecule.GetBonds():
        ...
        if bond.GetBondType() != Chem.BondType.DOUBLE or stereo not in _PLANAR_STEREO:
            continue
        ...
        for references, owner in (
            ...
            if len(references) == 1:
                virtual = _virtual_planar_reference(molecule.GetAtomWithIdx(owner))
...
def _cumulene_constraints(
    ...
    for support in _potential_cumulene_supports(molecule):
        ...
        constraints.append(LocalCumuleneOrientation(path, terminal_frames))
    return tuple(constraints)
...
def _preserves_tetrahedral_constraints(
    ...
    for source in constraints:
        ...
        if target is None:
            return False
        mapped = tuple(
            _relabel_reference(reference, mapping) for reference in source.references
        )
...
def _preserves_planar_constraints(
    ...
    for source in constraints:
        ...
        if target is None:
            return False
        mapped_frame = tuple(
            _relabel_reference(reference, mapping) for reference in source.frame
        )
        preserving_frames = {
...
def _preserves_cumulene_constraints(
    ...
    by_atoms = {frozenset(constraint.path): constraint for constraint in constraints}
    for source in constraints:
        ...
        mapped = (
            ...
            tuple(_relabel_reference(reference, mapping) for reference in source_frame),
        )
...
def mapping_preserves_local_orientations(
    ...
    return (
        _preserves_tetrahedral_constraints(
            mapping,
...
def _vf2pp_labeled_graph(
    ...
    for sequence, (left, right, attributes) in enumerate(graph.edges(data=True)):
...
def tetrahedral_stabilizer_result(
    ...
    *,
    ...
    for image_indices in permutations(range(4)):
        mapped_references = tuple(references[index] for index in image_indices)
        ...
        compatible = True
        for slot, (source, target) in enumerate(zip(references, mapped_references)):
