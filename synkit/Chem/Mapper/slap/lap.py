...
def _leader_edge_cost(orders_i, orders_p):
    ...
    if n == 0:
        return 0.0
    # Absolute scalar matching is Monge: sorted-to-sorted pairing is optimal.
    a = np.sort(np.asarray(orders_i + [0.0] * (n - na), dtype=float))
    b = np.sort(np.asarray(orders_p + [0.0] * (n - nb), dtype=float))
...
def dual_lap_lower_bound(lgp, binary):
    ...
    if B.shape[0] != n:
        return _INF
    if Counter(elem_r) != Counter(elem_p):
        return _INF
