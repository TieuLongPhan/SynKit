"""
Branch-and-bound exact solver for the uncertainty-region kernel.
...
def solve_kernel(  # noqa: C901
    ...
    """Solve a :class:`~mapper.exact.kernel.Kernel` exactly by branch-and-bound.

    Assigns each uncertain reactant atom to an element-compatible uncertain
    product atom and prunes with an admissible cost lower bound. Symmetry is not
    inferred from global node-orbit membership: orbit transitivity alone does
    not justify sorting assignments and can discard valid optima.
    ...
    def pair_cost(a: int, b: int, a2: int, b2: int) -> float:
        """Directed pair contribution without an O(n^4) lookup table."""
        i = kernel.r_idx[a]
        p = kernel.p_idx[b]
        j = kernel.r_idx[a2]
        q = kernel.p_idx[b2]
        return abs(A[i, j] - B[p, q]) + abs(A[j, i] - B[q, p])
    ...
    def incr_cost(a: int, b: int) -> float:
        ...
        for a2 in assigned_stack:
            c += pair_cost(a, b, a2, assigned_pos[a2])
        return c
