...
class ExactMapper:
    ...
    2. **Admissible lower bound** — at each node, solves a small LAP on the
       ...
       pruning threshold.
    Search intentionally does not infer symmetry from local fingerprints.
    Equal fingerprints do not prove that two atoms can be transposed by an
    automorphism, so such a rule can discard the true optimum.
