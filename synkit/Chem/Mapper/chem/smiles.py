...
def standardize_reaction_center_hydrogens(mapped_rxn_smiles):
    """Make non-center hydrogens implicit without renumbering heavy-atom AAM.

    The mapped ITS decides which explicit hydrogens participate in changed
    bonds. Those hydrogens remain explicit for a later local mapping pass;
    every other hydrogen is folded into its heavy-atom ``hcount``. Heavy-atom
    map-number sets on both sides are verified after conversion.
    """
    if not HAS_RDKIT:
        raise ImportError("RDKit is required for processing SMILES")
    if mapped_rxn_smiles.count(">>") != 1:
        raise ValueError("mapped reaction SMILES must contain exactly one '>>'")

    def heavy_maps(reaction):
        out = []
        for smiles in reaction.split(">>"):
            mol = _mol_from_smiles(smiles, remove_hs=False)
            if mol is None:
                raise ValueError(f"Could not parse reaction SMILES: {reaction!r}")
            out.append(
                {
    ...
    expected_maps = heavy_maps(mapped_rxn_smiles)
    from synkit.Graph.ITS.normalize_aam import NormalizeAAM

    standardized = NormalizeAAM().fit(
        mapped_rxn_smiles,
        fix_aam_indice=False,
    )
    if heavy_maps(standardized) != expected_maps:
        raise RuntimeError("hydrogen standardization changed heavy-atom AAM")
    return standardized
