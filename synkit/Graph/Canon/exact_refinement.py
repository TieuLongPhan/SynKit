...
class ExactRefinementMixin:
    ...
    def _affected_cell_indices(
        ...
        for node in changed_members:
            affected.update(
                neighbour for neighbour, _token in self._outgoing[node]
            )
            if self._directed:
                affected.update(
                    neighbour for neighbour, _token in self._incoming[node]
                )
        cell_index = {
            node: index
    ...
    def _partition_lower_bound(
        ...
        def minimum_token(
            ...
            if not candidates:
                raise RuntimeError(
                    "Canonical lower bound has no candidate edge."
                )
    ...
    def _select_component_target(
        ...
        component_ambiguous = tuple(
            ...
            and set(item[1]).issubset(active_component)
        )
    ...
    def _ordered_search_children(
        ...
        if (
            ...
            or len(target) * len(self._nodes)
            > _TRACE_PREVIEW_WORK_LIMIT
        ):
            return [
                ((), position, chosen, None)
    ...
    def _orbit_already_explored(
        ...
        chosen: Hashable,
        explored: list[
            tuple[Hashable, tuple[Any, ...], tuple[Hashable, ...]]
        ],
