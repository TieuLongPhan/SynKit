...
class ApproxMCSMatcher:
    ...
    def __init__(
        ...
        *,
        ...
        # wildcard_aware requires wildcards to stay in the graph for matching
        self.prune_wc: bool = False if wildcard_aware else prune_wc
    ...
    def _is_wildcard_node(self, data: Dict[str, Any]) -> bool:
        """Return True if this node is a wildcard atom."""
        el = data.get(self.element_key)
        wc = self.wildcard_element
        wc0 = wc[0] if isinstance(wc, tuple) else wc
        if isinstance(el, tuple):
            return el[0] == wc0
        return el == wc0

    def _node_compatible(self, pdata: Dict[str, Any], hdata: Dict[str, Any]) -> bool:
        """Node compatibility check, optionally treating wildcards as universal."""
        if self.wildcard_aware and (
            self._is_wildcard_node(pdata) or self._is_wildcard_node(hdata)
        ):
            return True
    ...
    def _node_similarity(
        ...
        * Optional 1-WL color agreement (if :pyattr:`use_wl` is ``True``).
        ...
        hdata = host.nodes[h]
        if not self._node_compatible(pdata, hdata):
            return -1.0
    ...
    def _bipartite_assign_components(
        ...
        *,
        ...
        total overlap, then returns the combined **G1 → G2** mapping for the
        optimally assigned pairs.

        Falls back to :py:meth:`_componentwise_approx` when scipy is not
        available.
        ...
        try:
            ...
            return self._componentwise_approx(
                G1, G2, max_seeds=max_seeds, max_steps=max_steps
            )
        ...
        # Build score matrix and cache results in one pass
        ...
        for i, sub1 in enumerate(comps1):
            for j, sub2 in enumerate(comps2):
                ...
                maps, sz = self._approximate_mappings(
                    pattern, host, max_seeds=max_seeds, max_steps=max_steps
                )
                score[i, j] = float(sz)
    ...
    def find_rc_mapping(
        ...
        * ``'its'`` → treat ``rc1`` and ``rc2`` directly as graphs
        ...
        if component:
            combined = self._bipartite_assign_components(
                G1_use,
                G2_use,
                max_seeds=max_seeds,
                max_steps=max_steps,
