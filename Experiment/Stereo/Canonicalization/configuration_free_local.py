...
def _carrier_result(
    *,
    ...
    for class_index, class_key in enumerate(sorted(theoretical)):
        ...
        collapsed = not issues and len(digests) == len(descriptors) and len(distinct) == 1
...
def _select_rows(
    ...
    *,
    ...
    return (
        [
            (f"RotA-{index:04d}", str(row["SMILES"]))
...
def benchmark(
    ...
    *,
    ...
    expected = sum(
        carrier["raw_local_permutations_expected"] for carrier in carriers
    )
    ...
    return {
        ...
        "summary": {
            ...
            "records_with_carriers": sum(bool(record["carriers"]) for record in records),
