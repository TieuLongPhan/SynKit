...
_CASES = (
    ...
    CaseSpec("A", "rota", "RotA-0071", _composition(atrop_bond=1, planar_bond=1), 64, 4),
    CaseSpec("A", "rota", "RotA-0073", _composition(atrop_bond=1, planar_bond=1), 64, 4),
...
def _carrier_inventory(molecule: Chem.Mol) -> tuple[Any, tuple[dict[str, Any], ...], dict[str, int]]:
...
class _RawProgress:
    ...
    def __init__(
        ...
        *,
        ...
        self._label = (
            f"raw {spec.budget} {spec.source}:{spec.record_id}"
        )
        self._total = total
    ...
    def enabled(self) -> bool:
        return (
            self._interval_seconds is not None
...
def _case_result(
    ...
    if formal_count > max_assignments:
        validation_issues.append(
            f"assignment_limit:{formal_count}>{max_assignments}"
        )
    public_carriers = [
        {
            key: value
    ...
    expected_formal_tuples = tuple(
        product(
            *(
                range(carrier["formal_configuration_classes"])
                for carrier in carriers
            )
        )
    )
    ...
    raw_multiplicities = sorted(
        result["represented_raw_assignment_multiplicity"]
        for result in global_classes
    )
    representation_invariance = (
        len(invariant_formal_tuples) == formal_count
    )
    passed = (
        complete
...
def _selected_rows(
    ...
    *,
    ...
    if "acs" in needed:
        ...
        rows.update(
            {
                ("acs", str(row["ID"])): str(row["Input SMILES"])
                for row in loaded
            }
        )
        integrity["acs"] = {
...
def _run_cases(
    items: list[
        tuple[CaseSpec, str, str, float, int, float | None]
    ],
    *,
    jobs: int,
) -> list[dict[str, Any]]:
...
def benchmark_budget(
    ...
    *,
    ...
    selected = tuple(
        case
        for case in specs
        if not case_ids or case.record_id in case_ids
    )
    ...
    formal_expected = sum(
        record.get("joint_formal_assignments_expected", 0)
        for record in records
    )
    enumerated_expected = sum(
        record.get("enumerated_assignments_expected", 0)
        for record in records
    )
    enumerated_completed = sum(
        record.get("enumerated_assignments_completed", 0)
        for record in records
    )
    return {
        ...
        "summary": {
            ...
            "configuration_dependent_supports": sum(
                record.get("configuration_dependent_supports", 0)
                for record in records
            ),
            ...
            "observed_global_canonical_classes": sum(
                record.get("observed_global_canonical_classes", 0)
                for record in records
            ),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="SECONDS",
        help=(
            "raw-mode terminal progress interval; use 0 to disable "
            "(default: 10)"
        ),
    )
    parser.add_argument(
