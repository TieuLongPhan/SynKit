...
def _element_signature(element: PotentialStereoElement) -> tuple[Any, ...]:
    ...
    if isinstance(support, AtomStereoSupport):
        ...
        raise TypeError(
            f"Unsupported carrier support: {type(support).__name__}"
        )
    return (
...
def _detect(
    ...
    *,
    ...
    issues = [
        *(
            f"missing:{value!r}"
            for value in sorted(expected - observed, key=repr)
        ),
        *(
            f"extra:{value!r}"
            for value in sorted(observed - expected, key=repr)
        ),
    ]
...
def _renumbering_summary(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    checked = [
        ...
        if record["renumbering_invariant"] is not None
    ]
    failures = [
    ...
    return {
        **_set_score(
        ...
        "records_with_additional_detected_loci": sum(
            bool(
                set(record[prediction_field])
                - set(record[reference_field])
            )
            for record in values
        ),
    }
...
def _erased_miss_classification(
    ...
    *,
    ...
    return {
        ...
        "unrecovered_with_supplied_local_neighbor_orientation": sum(
            remaining.values()
        ),
    ...
    *,
    ...
    for locus in sorted(missed_loci):
        ...
        if any(
            ...
            and constraint.center in component
            for constraint in constraints.tetrahedral
        ):
            frame_families.append("tetrahedral")
        if any(
            constraint.bond <= component
            for constraint in constraints.planar
        ):
            frame_families.append("planar")
        if any(
            set(constraint.path) <= component
            for constraint in constraints.cumulene
        ):
        ...
        details.append(
            {
                ...
                "ring_membership": (
                    "ring" if atom.IsInRing() else "acyclic"
                ),
...
def _erased_miss_detail_summary(
    ...
    return {
        ...
        "by_configuration_erased_status": dict(
            sorted(
                Counter(
                    detail["configuration_erased_status"]
        ...
        "by_atom_element": dict(
            sorted(
                Counter(
                    detail["atom_element"] for detail in details
                ).items()
            )
        ),
        "by_ring_membership": dict(
            sorted(
                Counter(
                    detail["ring_membership"] for detail in details
                ).items()
            )
        ),
        "by_available_local_neighbor_frame_families": dict(
            sorted(
                Counter(
                    "+".join(
                        detail[
                            "available_local_neighbor_frame_families"
                        ]
                    )
                    or "none"
                    for detail in details
    ...
    *,
    ...
    for position, row in enumerate(rows, start=1):
        ...
        retained_predictions = {
            _exact_locus(element) for element in retained_elements
        }
        retained_confirmed_predictions = {
            _exact_locus(element)
            for element in _confirmed(retained_elements)
        }
        results.append(
            {
                "id": row["ID"],
                ...
                "broad_missed_supplied_loci": sorted(
                    reference - broad_predictions
                ),
                "confirmed_missed_supplied_loci": sorted(
                    reference - confirmed_predictions
                ),
                "broad_additional_detected_loci": sorted(
                    broad_predictions - reference
                ),
    ...
    all_reference = sum(
        len(record["reference_supplied_loci"]) for record in parsed
    )
    modes = {
        mode: {
            **_reference_recall(
    ...
    for label in sorted({record["manual_global_label"] for record in valid}):
        subset = [
            record
            for record in valid
            if record["manual_global_label"] == label
        ]
    ...
    retained_prediction_field = (
        "local_neighbor_orientation_retained_predicted_loci"
    )
    return (
        {
            "task": (
                "two-setting recovery of supplied configured local loci"
            ),
            ...
            **modes,
            ...
            "configuration_erased_miss_structure": (
                _erased_miss_detail_summary(valid)
            ),
...
def _rota_predicted_loci(
    ...
    if chiral_type == "Chiral atom pair":
        ...
        return {
            ...
            if bond.GetBeginAtomIdx() in centers
            and bond.GetEndAtomIdx() in centers
        }
    if chiral_type == "Spiral atom and chain":
        rings = tuple(
            frozenset(ring) for ring in molecule.GetRingInfo().AtomRings()
        )
        centers = {
            atom.GetIdx()
        ...
        for component in nx.connected_components(chain_graph):
            terminals = sorted(
                ...
                if chain_graph.degree(center) == 1
            )
            if len(terminals) == 2:
...
def _rota_projection_issues(
    ...
    for mode, source, candidates in (
        ...
        issues.extend(
            f"{mode}_projection_extra:{value}"
    ...
    *,
    ...
    for position, row in enumerate(rows, start=1):
        ...
        if retained_invariant is not None:
            ...
            retained_invariant = (
                retained_invariant and not retained_projection_issues
            )
        broad_paths = {
        ...
        results.append(
            {
                ...
                "broad_missed_expanded_paths": sorted(
                    reference_paths - broad_paths
                ),
                ...
                (
                    "local_neighbor_orientation_retained_missed_"
                    "expanded_paths"
                ): sorted(reference_paths - retained_paths),
                "detected_axes": [
                    _serialize_element(element) for element in axes
                ],
                "detected_carriers": [
                    _serialize_element(element) for element in elements
                ],
    ...
    parsed = [record for record in results if not record["parse_failure"]]
    valid = [record for record in parsed if record["detection_error"] is None]
    all_reference_loci = sum(
        len(record["reference_pairs"]) for record in parsed
    )
    ...
    for chiral_type in sorted({record["chiral_type"] for record in valid}):
        subset = [
            record for record in valid if record["chiral_type"] == chiral_type
        ]
        by_type[chiral_type] = {
            "broad": _set_score(
                subset,
    ...
    return (
        {
            "task": (
                "two-setting positive typed stereo-carrier locus detection"
            ),
            "dataset_records": len(rows),
            "parsed_records": len(parsed),
            "evaluated_records": len(valid),
            ...
            "detected_axes_by_family": _prediction_counts(
                valid, "detected_axes"
            ),
            "detected_carriers_by_family": _prediction_counts(
...
def _cip_prediction_positions(
    ...
    for element in elements:
        ...
        if element.element_type is StereoElementType.TETRAHEDRAL:
            ...
            positions.update(
                f"planar:{position + 1}" for position in support.endpoints
            )
        elif element.element_type in _AXIS_TYPES:
            positions.update(
                f"axial:{position + 1}" for position in support.endpoints
            )
...
def _benchmark_cip(
    ...
    *,
    ...
    for position, row in enumerate(rows, start=1):
        ...
        reference_upper, reference_lower = _cip_reference_positions_by_case(
            row
        )
        try:
            elements, invariant, issues = _detect(
        ...
        results.append(
            {
                ...
                "neighbor_configuration_assisted_detected_carriers": [
                    _serialize_element(element)
                    for element in assisted_elements
                ],
                "renumbering_invariant": invariant,
                "renumbering_issues": issues,
                ...
                "neighbor_configuration_assisted_renumbering_issues": (
                    assisted_issues
                ),
            }
        )
    ...
    valid = [record for record in parsed if record["detection_error"] is None]
    all_reference = sum(
        len(record["reference_positions"]) for record in parsed
    )
    by_unit = {}
    categories = sorted(
    ...
    for category in categories:
        subset = [
            record for record in valid if category in record["stereo_units"]
        ]
        by_unit[category] = {
            "broad": _set_score(
                subset,
                ...
                "failures": [
                    {
                        ...
                        "issues": record[
                            (
                                "neighbor_configuration_assisted_"
                                "renumbering_issues"
                            )
                        ],
                    }
...
def benchmark_full_detection(
    ...
    *,
    ...
    exact_overlap = sum(
        acs_smiles[identifier] == cip_smiles[identifier]
    ...
    return {
        ...
        "protocol": {
            ...
            "two_setting_comparison": {
                "setting_1": (
                    "retain_configuration_as_local_neighbor_orientation"
                ),
