...
def _formal_graph(structure: dict[str, Any]) -> nx.Graph:
    ...
    graph.add_edges_from(
        ...
        for left, right, kind in structure["edges"]
    )
    return graph


def _helical_observation(structure: dict[str, Any]) -> tuple[str, dict[str, Any] | None]:
...
def _planar_observation(structure: dict[str, Any]) -> tuple[str, dict[str, Any] | None]:
    plane = tuple(node["id"] for node in structure["nodes"] if node.get("role") == "plane")
    pilots = tuple(node["id"] for node in structure["nodes"] if node.get("role") == "pilot")
    if len(pilots) != 1:
