...
def _orientation_witness(
    ...
    if atom_positions is None:
        raise ValueError(
            f"No non-coplanar 3D witness exists for {record_ids[0]}."
        )
    orientations = []
    for record_id in record_ids:
        ...
        if molecule.GetNumAtoms() <= max(atom_positions):
            raise ValueError(
                f"3D record {record_id} lacks the witness atom positions."
            )
    ...
    return {
        ...
        "atom_positions_one_based": [
            position + 1 for position in atom_positions
        ],
        "orientations": orientations,
        "distinguished": len(
            {item["orientation"] for item in orientations}
        )
        == len(orientations),
...
def _audit_coordinate_extension(
    ...
    if digest != CIP_3D_SHA256:
        raise ValueError(
            f"Unexpected CIP 3D Validation Suite SHA-256: {digest}"
        )
