...
def _transport_unique_registry(
    ...
    registry_keys = {
        frozenset(key(descriptor) for descriptor in registry) for registry in registries
    }
...
def benchmark_cip_native(
    ...
    *,
    ...
    coordinate_molecules = (
        {} if coordinate_path is None else _load_coordinate_molecules(coordinate_path)
    )
    results = []
    rdkit_results = []
    parse_failures = []
