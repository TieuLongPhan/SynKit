...
ROOT = HERE.parents[2]
DATA_ROOT = HERE / "Data"
RESULTS = DATA_ROOT / "bidirectional-replay"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=DATA_ROOT / "graph-rewriting-comparison",
...
def plot_panel(
    ...
    for position, direction in enumerate(("forward", "backward")):
        ...
        axis.text(
            change + (0.012 * span if change >= 0 else -0.012 * span),
            position - 0.30,
            f"{llg:,}",
        ...
        signed_change = f"+{change:.2f}" if change >= 0 else f"−{abs(change):.2f}"
        axis.text(
            change / 2,
            position + 0.24,
            f"{sign}{abs(difference):,}  ({signed_change}%)",
            ha="center",
            va="top",
            fontsize=8.2,
            color=MUTED,
