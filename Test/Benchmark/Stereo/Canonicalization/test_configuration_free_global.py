...
REPORT_ROOT = ROOT / "Experiment" / "Stereo" / "Data" / "Canonicalization" / "Global"
...
def test_vs191_factorizes_to_128_joint_assignments():
    ...
    assert prod(carrier["formal_configuration_classes"] for carrier in carriers) == 128
    assert sum(carrier["configuration_dependent_support"] for carrier in carriers) == 1
...
def test_raw_and_formal_modes_produce_the_same_global_quotient(capsys):
    ...
    assert {result["canonical_digest"] for result in formal["global_classes"]} == {
...
def test_retained_abc_matrix_includes_completed_raw_stress_runs():
    paths = {
        ...
        ("A", "raw"): (REPORT_ROOT / "A" / "global_raw_canonicalization_report.json"),
        ("B", "formal"): (
