...
def _tetrahedral_fixture():
    return next(
        fixture for fixture in _fixture_catalog() if fixture.family == "tetrahedral"
    )
...
def test_supplied_tetrahedral_orientation_does_not_change_local_result():
    clockwise = _molecular_record(("test", "clockwise", "F[C@](Cl)(Br)I", 10.0))
...
def test_multiple_centres_are_benchmarked_independently():
    record = _molecular_record(("test", "two-centres", "F[C@H](Cl)[C@@H](Br)I", 10.0))
    ...
    assert (
        sum(
            carrier["raw_local_permutations_expected"] for carrier in record["carriers"]
        )
        == 48
    )
...
def test_retained_local_matrix_is_complete():
    ...
    for task in ("internal", "acs", "cip", "rota"):
        ...
        totals["theoretical_classes"] += summary["theoretical_configuration_classes"]
