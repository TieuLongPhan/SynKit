...
def _integer_references(value: Any) -> list[int]:
    ...
    if isinstance(value, list):
        return [reference for item in value for reference in _integer_references(item)]
...
def test_case_matrix_covers_every_family_and_information_state_once() -> None:
    ...
    assert {(case["family"], case["case_type"]) for case in cases} == {
        (family, case_type) for family in FAMILIES for case_type in CASE_TYPES
    }
...
def test_perception_runner_erases_configuration_before_carrier_detection() -> None:
    tetrahedral = neutralize_rdkit_configuration(Chem.MolFromSmiles("F[C@H](Cl)Br"))
    double_bond = neutralize_rdkit_configuration(Chem.MolFromSmiles("F/C(Cl)=C(/Br)I"))
    assert (
        detect_potential_stereo_elements(tetrahedral)[0].configuration_state
        is StereoConfigurationState.UNSPECIFIED
    )
    assert (
        detect_potential_stereo_elements(double_bond)[0].configuration_state
        is StereoConfigurationState.UNSPECIFIED
    )
...
def test_frozen_conformance_report_matches_the_live_stable_result() -> None:
    ...
    assert [
        {key: value for key, value in record.items() if key not in {"seconds"}}
