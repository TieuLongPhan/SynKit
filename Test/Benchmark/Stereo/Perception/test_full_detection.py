...
def test_large_rota_symmetry_case_completes_with_exact_statuses() -> None:
    ...
    molecule = neutralize_rdkit_configuration(
...
def test_preserved_focal_tag_cannot_prove_repeated_ligands_distinct() -> None:
    ...
    assert molecule is not None
    assert (
        molecule.GetAtomWithIdx(1).GetChiralTag()
...
def test_frozen_report_covers_every_row_and_every_source_annotation() -> None:
    ...
    assert acs["by_reference_family"]["tetrahedral"]["broad"][
        "reference_loci"
    ] == 916
    assert acs["by_reference_family"]["double_bond"]["broad"][
        "reference_loci"
    ] == 91
    assert acs["broad"]["recovered_loci"] == 743
    assert (
        sum(
            len(record["broad_missed_supplied_loci"])
            for record in acs["records"]
        )
        == 264
    )
    assert acs["two_setting_comparison"]["configuration_erased"][
        "true_positive_loci"
    ] == 743
    assert acs["two_setting_comparison"][
        "local_neighbor_orientation_retained"
    ]["true_positive_loci"] == 1007
    assert acs["two_setting_comparison"][
        "local_neighbor_orientation_retained"
    ]["additional_detected_loci"] == 109
    ...
    assert not acs["local_neighbor_orientation_retained_renumbering"][
        "failures"
    ]
    ...
    assert not any(
        record["broad_missed_expanded_paths"] for record in rota["records"]
    )
    for setting in (
        "configuration_erased",
        "local_neighbor_orientation_retained",
    ):
        assert rota["two_setting_comparison"][setting][
            "true_positive_loci"
        ] == 698
    ...
    assert not rota["local_neighbor_orientation_retained_renumbering"][
        "failures"
    ]
    ...
    assert (
        cip["by_reference_class"]["tetrahedral"]["broad"]["reference_loci"]
        == 980
    )
    assert (
        cip["by_reference_class"]["planar"]["broad"]["reference_loci"] == 232
    )
    assert cip["by_reference_class"]["axial"]["broad"]["reference_loci"] == 40
    assert (
        cip["by_reference_case"]["uppercase"]["broad"]["reference_loci"]
        == 1044
    )
    assert (
        cip["by_reference_case"]["lowercase"]["broad"]["reference_loci"] == 208
    )
    assert cip["broad"]["true_positive_loci"] == 969
    assert cip["confirmed"]["true_positive_loci"] == 959
    assert cip["by_reference_class"]["axial"]["broad"][
        "recovered_loci"
    ] == 40
    ...
    assert cip["by_reference_class"]["tetrahedral"][
    ...
    assert not Counter(
        ...
        for position in record[
            "neighbor_configuration_assisted_missed_positions"
        ]
    )
    assert not cip["neighbor_configuration_assisted_renumbering"]["failures"]
    assert cip["two_setting_comparison"]["configuration_erased"][
        "additional_detected_loci"
    ] == 19
    assert cip["two_setting_comparison"][
        "local_neighbor_orientation_retained"
    ]["additional_detected_loci"] == 57
...
def test_report_contains_no_cip_structures_or_configuration_predictions() -> (
    None
):
