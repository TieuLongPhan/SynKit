...
def test_framework_value_validates_and_round_trips_without_provenance_identity() -> (
    None
):
...
def test_framework_frame_ordering_and_relabeling_are_exact() -> None:
    ...
    assert (
        relabeled.relabel({value: key for key, value in mapping.items()}) == descriptor
    )
...
def test_framework_certificate_survives_deterministic_relabeling_matrix() -> None:
    ...
    atoms = sorted(graph)
    permutations = [atoms[offset:] + atoms[:offset] for offset in range(len(atoms))]
