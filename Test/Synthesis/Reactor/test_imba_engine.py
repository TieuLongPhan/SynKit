...
class TestImbaEngine(unittest.TestCase):
    def setUp(self):
        ...
        self.smart = (
            ...
            "[cH:2]1[c:20]([C:22]([OH:7])=[O:21])[s:18][c:24]([S:6][c:29]2[c:15]([Cl:26])[cH:8][n:19][cH:9][c:16]2[Cl:27])[c:30]1[N+:5]([O-:3])=[O:13]>>"
            "[cH:1]1[cH:14][c:10]2[c:23]([cH:11][n:25]1)[cH:17][cH:12][cH:4][c:31]2[NH:28][C:22]([c:20]1[cH:2][c:30]([N+:5]([O-:3])=[O:13])[c:24]([S:6][c:29]2[c:15]([Cl:26])[cH:8][n:19][cH:9][c:16]2[Cl:27])[s:18]1)=[O:21]"
        )
    ...
    def test_pipeline_forward(self):
        ...
        out = engine.smarts_list
        self.assertEqual(Standardize().fit(out[0], remove_aam=True), self.rsmi)
    ...
    def test_pipeline_backward(self):
        ...
        engine = ImbaEngine(
            substrate_p, its, add_wildcard=True, clean_fragments=False, invert=True
        )
        out = engine.smarts_list
        ...
        engine_clean = ImbaEngine(
            substrate_p, its, add_wildcard=True, clean_fragments=True, invert=True
        )
        out_clean = engine_clean.smarts_list
        self.assertEqual(len(out_clean), 2)
        cleaned_rsmi = Standardize().fit(out_clean[0], remove_aam=True)
        self.assertEqual(cleaned_rsmi, self.rsmi)
