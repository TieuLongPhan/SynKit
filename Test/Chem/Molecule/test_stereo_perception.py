...
def test_isotope_distinguished_alkene_is_detected_without_configuration() -> None:
    ...
    assert double_bonds[0].configuration_state is StereoConfigurationState.UNSPECIFIED
...
def test_symmetric_cumulene_remains_a_typed_broad_candidate() -> None:
    ...
    assert tuple(element.identifier for element in elements) == ("cumulene_axis:1-3-4",)
...
def test_symmetric_biaryl_remains_a_typed_broad_candidate() -> None:
    elements = detect_potential_stereo_elements(_molecule("c1ccccc1-c1ccccc1"))
...
def test_angular_five_ring_helicene_is_a_configuration_free_path_carrier() -> None:
...
def test_center_stabilizer_parity_not_four_singleton_orbits_is_criterion() -> None:
    references = (1, 2, 3, 4)
